...
var projects = [];
var apiOnline = false; // toggled by API status checker
...
(function () {
  ...
  const apiStatusEl = typeof document !== "undefined" ? document.getElementById("apiStatus") : null;
  const updateApiStatus = (state, text) => {
    if (!apiStatusEl) return;
    apiStatusEl.classList.remove("ok", "error");
    if (state === "ok") apiStatusEl.classList.add("ok");
    if (state === "error") apiStatusEl.classList.add("error");
    const label = apiStatusEl.querySelector(".label");
  ...
  async function checkApi() {
    if (!apiStatusEl) return;
    try {
      updateApiStatus(null, "API: checking…");
      ...
      if (data && data.message) {
        apiOnline = true;
        if (typeof window !== "undefined") window.apiOnline = true;
        updateApiStatus("ok", "API: online");
      } else {
        apiOnline = false;
        if (typeof window !== "undefined") window.apiOnline = false;
        updateApiStatus("error", "API: unexpected");
      }
    } catch (e) {
      apiOnline = false;
      if (typeof window !== "undefined") window.apiOnline = false;
  ...
  function badgeClass(s) {
    ...
    if (s === "in progress" || s === "inprogress" || s === "progress") return "blue";
  ...
  function normalizeStatus(s) {
    s = lc(s);
    if (s === "inprogress" || s === "progress" || s === "in progress") return "progress";
  ...
  function updateActivityLog() {
    ...
    if (selectedTeamMembers.length > 0) {
      activities = activities.filter(function (activity) {
        ...
        return activity.assignedTo && selectedTeamMembers.includes(activity.assignedTo);
      });
    }
    ...
    recentActivities.forEach(function (activity, index) {
      ...
      item.addEventListener("click", function () {
        ...
        setTimeout(function () {
          ...
          projectCards.forEach(function (card) {
            ...
            if (titleElement && titleElement.textContent.trim() === activity.projectName.trim()) {
      ...
      dot.className =
        "activity-dot " +
        (index === 0 ? "recent" : index === 1 ? "medium" : index <= 2 ? "older" : "oldest");
  ...
  function generateProjectActivities(project) {
    ...
    for (var i = 0; i < numActivities; i++) {
      ...
      var activityDate = new Date(baseDate.getTime() - i * Math.random() * 7 * 24 * 60 * 60 * 1000); // Up to 7 days ago
      ...
      if (project.team && Array.isArray(project.team) && project.team.length > 0) {
        assignedMember = project.team[Math.floor(Math.random() * project.team.length)];
      }
  ...
  function updateTasksCard(filteredProjects) {
    ...
    var pctAll = totals.total ? Math.round((100 * totals.done) / totals.total) : 0;

    document.getElementById("tasksSummary").textContent = totals.total;
    document.getElementById("tasksMeta").textContent = totals.done + " / " + totals.total + " done";
    ...
    recentActivities.forEach(function (activity, index) {
      ...
      item.addEventListener("click", function () {
        ...
        setTimeout(function () {
          ...
          projectCards.forEach(function (card) {
            var titleElement = card.querySelector(".title");
            if (titleElement && titleElement.textContent.trim() === activity.projectName) {
              card.scrollIntoView({ behavior: "smooth", block: "center" });
  ...
  function updateWheelState() {
    ...
    [activeArc, progressArc, blockedArc, holdArc, otherArc].forEach(function (arc) {
      arc.classList.remove("arc-active");
    });

    // Add active class to current filter
    if (currentFilter === "active") activeArc.classList.add("arc-active");
    else if (currentFilter === "progress") progressArc.classList.add("arc-active");
    else if (currentFilter === "blocked") blockedArc.classList.add("arc-active");
  ...
  function renderProjectCards(projectsToRender) {
    ...
    if (!projectsToRender.length) {
      ...
      if (currentFilter || currentSearchTerm || selectedTeamMembers.length > 0) {
      ...
      projectsToRender.forEach(function (p) {
        ...
        meta.className = "meta";
        meta.textContent = done + " / " + total + " tasks • Updated " + es(p.updated || "—");
        ...
        function handleCardClick() {
          ...
          var url =
            "fmp://$/template.fmp12?script=project%20-%20open&param=" + encodeURIComponent(id);
  ...
  // Try to hydrate projects from backend API, fallback to embedded data
  async function hydrateProjectsFromApi() {
    try {
      ...
      // Keep fallback projects already parsed from #data
      if (typeof updateDisplay === "function") updateDisplay();
      if (typeof updateAnalyticsData === "function") updateAnalyticsData();
    }
  }

  // Kick off hydration once DOM is ready (script is at end of body, so it's safe)
  ...
  if (projects.length > 0) {
    ...
    if (counts.hold > 0) segments.push({ count: counts.hold, arc: holdArc, status: "hold" });
    if (counts.other > 0) segments.push({ count: counts.other, arc: otherArc, status: "other" });
    ...
    if (segments.length > 0) {
      ...
      if (useMinSizes) {
        ...
        segments.forEach(function (segment, index) {
          segment.arc.setAttribute("stroke-dasharray", minSegmentSize + " " + circumference);
          ...
          segment.arc.addEventListener("click", function (e) {
            e.stopPropagation();
            handleArcClick(currentFilter === segment.status ? null : segment.status);
          });
        });
      } else {
        // Use proportional sizes but ensure minimum
        segments.forEach(function (segment, index) {
          var proportionalLength = (segment.count / total) * availableCircumference;
          var length = Math.max(proportionalLength, minSegmentSize);

          segment.arc.setAttribute("stroke-dasharray", length + " " + circumference);
          segment.arc.setAttribute("stroke-dashoffset", -offset);
  ...
  if (!searchInput || !clearButton) {
    ...
    // Search input functionality (debounced)
...
function showModal(title, content, onConfirm = null) {
  ...
  // Focus management
  const firstFocusable = overlay.querySelector("button, input, textarea, select");
...
function clearAllFilters() {
  ...
  // Clear team checkboxes
  var teamCheckboxes = document.querySelectorAll('#teamOptionsContainer input[type="checkbox"]');
  teamCheckboxes.forEach(function (cb) {
    cb.checked = false;
  });
...
function initializeNavigation() {
  ...
  if (navDropdownTrigger && navDropdownMenu) {
    ...
    // Dropdown actions
    document.querySelectorAll(".nav-dropdown-link[data-action]").forEach((link) => {
      link.addEventListener("click", function (e) {
        e.preventDefault();
  ...
  // Close dropdown when clicking outside
  document.addEventListener("click", function (e) {
    if (!navDropdownTrigger?.contains(e.target) && !navDropdownMenu?.contains(e.target)) {
      navDropdownMenu?.classList.remove("show");
      navDropdownTrigger?.setAttribute("aria-expanded", "false");
    }
  });
...
function applyFilterPreset(preset) {
  ...
  switch (preset) {
    ...
    case "my-projects": {
      ...
      showToast("Filter Applied", `Showing projects assigned to ${currentUser}`, "success", 3000);
      break;
    }
    case "overdue":
      ...
      showToast("Filter Applied", "Showing overdue and blocked projects", "warning", 3000);
      break;
    case "this-quarter":
      ...
      showToast("Filter Applied", "Showing active projects from this quarter", "info", 3000);
      break;
  }
}
...
function initializeQuickActions() {
  ...
  document.getElementById("exportBtn")?.addEventListener("click", function () {
    ...
    setTimeout(() => {
      hideLoading(this);
      showToast("Export Complete", "Project data has been exported to CSV", "success");
    }, 2000);
  });
...
window.clearAdvancedSearch = function () {
  ...
  // Reset checkboxes
  document.querySelectorAll('#searchInFields input[type="checkbox"]').forEach((cb) => {
...
window.applyAdvancedSearch = function () {
  ...
  // Show feedback
  showToast("Search Applied", "Advanced search filters have been applied", "success");
...
window.saveCurrentFilter = function () {
  ...
  if (filterName) {
    ...
    loadFilterHistory();
    showToast("Filter Saved", `Filter "${filterName}" has been saved`, "success");
  }
};
  ...
  if (currentSearchTerm || currentFilter || selectedTeamMembers.length > 0) {
    ...
    const isDuplicate = filterHistory.some(
      (item) =>
        ...
        JSON.stringify(item.teamMembers) === JSON.stringify(historyItem.teamMembers)
    );
  ...
  if (recentSearches) {
    ...
    if (filterHistory.length === 0) {
      ...
      filterHistory.slice(0, 10).forEach((item) => {
        ...
        if (item.statusFilter) description.push(getStatusDisplayName(item.statusFilter));
        if (item.teamMembers.length) description.push(`${item.teamMembers.length} team member(s)`);

        historyItem.innerHTML = `
          <span>${description.join(" • ") || "All projects"}</span>
          <small style="color: var(--muted);">${formatRelativeTime(item.timestamp)}</small>
        `;
    ...
    if (savedFilters.length === 0) {
      ...
      savedFilters.forEach((filter) => {
        ...
        filterItem.innerHTML = `
          ...
          <small style="color: var(--muted);">${formatRelativeTime(filter.timestamp)}</small>
        `;
...
document.addEventListener("DOMContentLoaded", function () {
  // Initialize the display with projects data
  ...
  window.switchView = function (viewName) {
    ...
    switch (viewName) {
      case "overview":
        targetView = document.querySelector('main[role="main"]:not(.view-container)');
  ...
  function updateAnalyticsData() {
    ...
    const activeProjects = projects.filter((p) => normalizeStatus(p.status) === "active").length;
    // (Optional) Add more metrics here if needed in the UI
    const avgCompletion = Math.round(
      projects.reduce((sum, p) => sum + (p.done / p.total) * 100, 0) / totalProjects
    );
    const uniqueTeamMembers = new Set();
    projects.forEach((p) => p.team.forEach((member) => uniqueTeamMembers.add(member)));
    ...
    chartItems.forEach((item, index) => {
      if (fallbackData[index]) {
        ...
        if (fill) {
          ...
          console.log(`Set status bar width ${index}: ${fallbackData[index].percentage}%`);
        }
      }
    });
  }
  ...
  function showFallbackTimeline() {
    ...
    console.log("Fallback timeline populated with", fallbackTimeline.length, "items");
  }
  ...
  window.animateAnalyticsData = function () {
    ...
    setTimeout(() => {
      const summaryFills = document.querySelectorAll(".summary-fill[data-width]");
  ...
  window.refreshAnalytics = function () {
    ...
    const allFills = document.querySelectorAll(".status-fill, .progress-fill, .summary-fill");
